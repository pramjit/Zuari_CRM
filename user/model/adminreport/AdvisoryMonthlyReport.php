...
class ModeladminreportAdvisoryMonthlyReport extends Model {
	...
	LEFT JOIN mas_callstatus CS ON(crm_adv.CALL_STATUS=CS.STATUS_ID)
        ...
        AND DATE(CR_DATE) BETWEEN '".$fdate."' AND '".$tdate."'
        GROUP BY CASE_PIN
        ORDER BY DATE(CR_DATE)";
        $query = $this->db->query($sql);
        return $query->rows;
    }
	...
	LEFT JOIN mas_callstatus CS ON(crm_adv.CALL_STATUS=CS.STATUS_ID)
        ...
        AND DATE(CALL_DATE) BETWEEN '".$fdate."' AND '".$tdate."'
		GROUP BY CASE_PIN
