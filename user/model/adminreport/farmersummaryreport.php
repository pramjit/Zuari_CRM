...
class Modeladminreportfarmersummaryreport extends Model {
    ...
    public function  MobileList($data){
        ...
        $to_date=$data["to_date"];
        $sql="SELECT CC.MOBILE,CC.STATE,MP.`NAME` AS 'STATE_NAME', CC.DATE_RECEIVED,CC.`STATUS`,CC.CALL_TYPE,
        SUM(CASE WHEN CC.CC_LIVE=0 THEN 1 ELSE 0 END) AS 'NEW',
        SUM(CASE WHEN CC.CC_LIVE=1 THEN 1 ELSE 0 END) AS 'EXISTING' ,
        SUM(CASE WHEN CC.CC_LIVE=2 THEN 1 ELSE 0 END) AS 'PENDING' 
        FROM (
        SELECT * FROM cc_incomingcall WHERE CALL_TYPE=2 AND DATE_RECEIVED BETWEEN '".$from_date."' AND '".$to_date."' GROUP BY MOBILE
        ) AS CC
        LEFT JOIN mas_pol_geo MP ON(CC.STATE=MP.GEO_ID)
        GROUP BY CC.STATE";    
        $query = $this->db->query($sql);
