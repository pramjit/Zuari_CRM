...
class Modeladminreportcomplaintdetailreport extends Model {
    ...
    public function  ComplaintList($data){
        $from_date=$data["from_date"];
        $to_date=$data["to_date"];
        $sql="SELECT A.CASE_ID,A.COMP_MOBILE,
        SUM(CASE WHEN A.CASE_STATUS IN (3,7) THEN 1 ELSE 0 END) AS 'PEN_RA',
        SUM(CASE WHEN A.CASE_STATUS IN (2,6) THEN 1 ELSE 0 END) AS 'PEN_AA',
