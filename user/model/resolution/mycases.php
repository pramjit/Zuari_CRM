...
class Modelresolutionmycases extends Model {
    
    public function getmissedcallData($data){
            $cr_by = $this->customer->getId();
            $log=new Log("RA_MyCases.log");
            $log->write("RA ID: ".$cr_by);
            ...
            $sql="SELECT cc.CASE_ID AS 'CASE_ID',
                ...
                WHERE cc.CASE_STATUS IN(1,6,7) and cc.COMP_RA='".$cr_by."'
                ORDER BY cc.CR_DATE";
            $log->write("RA_MyCase: ".$sql);
    ...
    public function CaseCurUser($cid){
        $sql="SELECT CONCAT(firstname,' ',lastname) as 'user' from ak_customer WHERE customer_id='".$cid."'";
        $query = $this->db->query($sql);
  ...
  public function getmissedcallDatacount($data){
    ...
    public function ProdData($caseid){
        ...
        $sql="SELECT A.COM_ST,A.COM_DT,A.COM_ID,A.COM_CAT,A.COM_TYP,A.COM_TXT,A.COM_CR_USR,A.COM_STS,B.UP_USER_ID,B.UP_FILE_PATH 
        FROM (
	(SELECT 
        mpgs.`name` AS 'COM_ST',
	mpgd.`name` AS 'COM_DT',
        ...
        CONCAT(akc.firstname,'',akc.lastname) AS 'COM_CR_USR',
        ...
        LEFT JOIN ak_farmer akf ON(cc.CASE_ID=akf.FAR_ID and cc.COMP_MOBILE=akf.FAR_MOBILE)
	LEFT JOIN mas_pol_geo mpgs ON(akf.STATE_ID=mpgs.GEO_ID)
	LEFT JOIN mas_pol_geo mpgd ON(akf.DISTRICT_ID=mpgd.GEO_ID)
    ...
    public function subradata($caseid,$commnts,$status,$filepath){
        ...
        if($ret_id==1){
            $Aa_Due_Date=date('Y-m-d', strtotime("+2 days"));
            $sql="update crm_case set CASE_STATUS='".$status."', AA_DUE_DATE='".$Aa_Due_Date."' where case_id='".$caseid."'";
            $this->db->query($sql);
    ...
    public function CaseMailDtls($caseid){
        $log=new Log("AA_MAIL_DETAILS".date('Ymd').".log");
        $sql="select crm_case.case_id as 'CASE_ID',crm_case.COMP_AA as 'AA_ID',
DATEDIFF(crm_case.AA_DUE_DATE,crm_case.RA_DUE_DATE) as 'NO_DAY' ,
(CURDATE() + INTERVAL DATEDIFF(crm_case.AA_DUE_DATE,crm_case.RA_DUE_DATE) DAY) AS 'DUE_DATE',
ak_customer.email as 'AA_MAIL',
concat(ak_customer.firstname,' ',ak_customer.lastname)as 'AA_NAME',
 2 AS 'FLAG' 
from crm_case 
left join ak_customer on(crm_case.COMP_AA=ak_customer.customer_id)
where crm_case.case_id='".$caseid."'";
        $log->write($sql);
        $query = $this->db->query($sql);
        return $query->row;
    }
