...
class Modelapprovaldashboard extends Model {
  ...
  public function getmissedcallDatacount($data){
    ...
    public function TotalcallData(){
        ...
        COUNT(CASE WHEN case_status <>0 AND COMP_AA='".$cr_by."' THEN case_status END) AS 'MYCASE',
        COUNT(CASE WHEN case_status = 2 THEN case_status END) AS 'PENDING',
        COUNT(CASE WHEN case_status = 8 THEN case_status END) AS 'CLOSED',
        COUNT(CASE WHEN case_status NOT IN(0,8,2) THEN case_status END) AS 'PROGRESS'
        FROM crm_case";
        $query = $this->db->query($sql);
        return $query->row;
    }
