...
AND CALL_COUNT<3 GROUP BY CASE_PIN ORDER BY DATE(CR_DATE) DESC";
    ...
    public function updateStatus($data)
    {
		...
		//********************* Check Call Status *************************//
		if(($caseSt==5)||($caseSt==12)||($caseSt==13)||($caseSt==14)){//Single Status...
			$sql="update crm_adv SET CASE_STATUS='0', CALL_STATUS='".$caseSt."', CALL_COUNT=CALL_COUNT+1 where FAR_MOB='".$mobile."'";
			$log->write($sql);
			$query = $this->db->query($sql);
			$suc = $this->db->countAffected();
			if($suc==1){
		...
		if(($caseSt==4)||($caseSt==6)||($caseSt==11)||($caseSt==22)||($caseSt==23)){//Busy,Notreachable etc...
			$sql="update crm_adv SET CASE_STATUS='".$caseSt."', CALL_STATUS='".$caseSt."', CALL_COUNT=CALL_COUNT+1 where FAR_MOB='".$mobile."'";
			$log->write($sql);
			$query = $this->db->query($sql);
			$suc = $this->db->countAffected();
			if($suc==1){
