...
class Modeladvisoryadvisory extends Model {
	...
	$cr_by = $this->customer->getId();
      $sql="select CASE_ID,FAR_MOB,CASE_PIN,CALL_COUNT, DATE_FORMAT(CR_DATE,'%d-%m-%Y %H:%i:%s') AS CR_DATE from crm_adv where case_status=7 and adv_id='".$cr_by."' ORDER BY CR_DATE DESC ";
    $query = $this->db->query($sql);
    return $query->rows;   
        
    }
    public function CallSts(){
        $sql="select STATUS_ID,STATUS_NAME from mas_callstatus where ACT=1 and STATUS_ID NOT IN(2,29,30,31,32,33,34)";
        $query = $this->db->query($sql);
        return $query->rows;   
    }
