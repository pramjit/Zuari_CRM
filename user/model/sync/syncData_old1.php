...
$data=$query->rows; 
    ...
    $data=$query->rows; 
     ...
     $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "geo where geo_type=4 ");
     $data=$query->rows; 
} else {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "geo where geo_type=4 and state_id In(SELECT GEO_ID  from " . DB_PREFIX . "customer_emp_map where EMP_ID ='".$id."' and GEO_LEVEL_ID='2')");
    $data=$query->rows; 
...
if($role_id1!='55' || $role_id1!='71' || $role_id1!='47'){
//save data product
...
$data=$query->rows;  
    ...
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "village ");
$data=$query->rows;  
} else {
...
// save mas_can_baseline_dtl
    $query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "mas_can_baseline_dtl limit 10");
    $data=$query->rows;
...
// save farmer_product_detail
    $query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "farmer_product_detail");
    $data=$query->rows;
...
//save data customer
$query = $this->db->query("SELECT customer_id,firstname,lastname,customer_group_id,district_id  FROM " . DB_PREFIX . "customer order by firstname asc");
$data=$query->rows; 
foreach ($data as $value){
