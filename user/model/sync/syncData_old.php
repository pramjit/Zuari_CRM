        ...
        
{
 ...
 $log->write("error");
    ...
    $file_db->exec("DROP TABLE IF EXISTS " . DB_PREFIX . "status");
...
$file_db->exec("CREATE TABLE IF NOT EXISTS " . DB_PREFIX . "status (
...
//state
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "geo WHERE  ACT ='1' and SID in (SELECT state_id from " . DB_PREFIX . "customer where customer_id ='".$id."'  ) order by GEO_NAME asc");
$data=$query->rows; 
...
//dist
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "geo WHERE  ACT ='1' and SID in (SELECT district_id from " . DB_PREFIX . "customer where customer_id ='".$id."'  ) order by GEO_NAME asc");
$data=$query->rows; 
foreach ($data as $value){
    $insert="insert into ".DB_PREFIX . "geo (SID,GEO_NAME,GEO_TYPE,Nation_ID,STATE_ID,ACT,statusl) values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["GEO_NAME"])."','".$mcrt->encrypt($value["GEO_TYPE"])."','".$mcrt->encrypt($value["Nation_ID"])."','".$mcrt->encrypt($value["STATE_ID"])."','".$mcrt->encrypt($value["ACT"])."','0')";
    $stmt = $file_db->prepare($insert);
...
//hq
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "geo WHERE  ACT ='1' and SID in (SELECT hq_id from " . DB_PREFIX . "customer where customer_id ='".$id."'  ) order by GEO_NAME asc");
$data=$query->rows; 
foreach ($data as $value){
    $insert="insert into ".DB_PREFIX . "geo (SID,GEO_NAME,GEO_TYPE,Nation_ID,STATE_ID,ACT,statusl) values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["GEO_NAME"])."','".$mcrt->encrypt($value["GEO_TYPE"])."','".$mcrt->encrypt($value["Nation_ID"])."','".$mcrt->encrypt($value["STATE_ID"])."','".$mcrt->encrypt($value["ACT"])."','0')";
    $stmt = $file_db->prepare($insert);
...
//save data village
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "village where HQ_ID IN (SELECT hq_id  FROM " . DB_PREFIX . "customer where customer_id='".$id."')");
$data=$query->rows; 
foreach ($data as $value){
...
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "status");
$data=$query->rows; 
foreach ($data as $value){
    $insert="insert into ".DB_PREFIX . "status (SID,STATUS,ACT,COLOR_CODE,statusl) values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["RETAILER_TYPE"])."','".$mcrt->encrypt($value["ACT"])."','".$mcrt->encrypt($value["COLOR_CODE"])."','0')";
    $stmt = $file_db->prepare($insert);
    $stmt->execute();
}
