        ...
        
{
 ...
 $log->write("error");
    ...
    $file_db->exec("DROP TABLE IF EXISTS " . DB_PREFIX . "all_district"); 
...
$file_db->exec("CREATE TABLE IF NOT EXISTS " . DB_PREFIX . "all_district (
...
if($role_id1=='72'){
    $query = $this->db->query("SELECT * FROM `ak_customer` where customer_group_id=47");
} else if($role_id1=='64'){ 
$query = $this->db->query("select * from (SELECT *  FROM ak_customer WHERE  status <>'11' and customer_group_id in (64,65,68) and customer_id in(SELECT CUSTOMER_ID FROM `ak_customer_map` where PARENT_CUSTOMER_ID='".$id."' ) 
UNION ALL
SELECT *  FROM ak_customer WHERE  status <>'11' and customer_group_id in (67) and  user_id in(select FAR_MOBILE from ak_farmer where CR_BY IN(SELECT CUSTOMER_ID FROM `ak_customer_map` where PARENT_CUSTOMER_ID='".$id."'))) a order by firstname asc");
  } else if($role_id1=='68'){ 
$query = $this->db->query("select * from (SELECT *  FROM ak_customer WHERE  status <>'11' and customer_group_id in (65,68) and customer_id in(SELECT CUSTOMER_ID FROM `ak_customer_map` where PARENT_CUSTOMER_ID='".$id."' ) 
UNION ALL
SELECT *  FROM ak_customer WHERE  status <>'11' and customer_group_id in (67) and  user_id in(select FAR_MOBILE from ak_farmer where CR_BY IN(SELECT CUSTOMER_ID FROM `ak_customer_map` where PARENT_CUSTOMER_ID='".$id."'))) a order by firstname asc");
  } else if($role_id1=='65'){ 
$query = $this->db->query("select * from (SELECT *  FROM ak_customer WHERE  status <>'11' and customer_group_id in (65) and customer_id in(SELECT CUSTOMER_ID FROM `ak_customer_map` where PARENT_CUSTOMER_ID='".$id."' ) 
UNION ALL
SELECT *  FROM ak_customer WHERE  status <>'11' and customer_group_id in (67) and  user_id in(select FAR_MOBILE from ak_farmer where CR_BY IN(SELECT CUSTOMER_ID FROM `ak_customer_map` where PARENT_CUSTOMER_ID='".$id."'))) a order by firstname asc");
  }
$data=$query->rows; 
...
//all district
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "geo WHERE  ACT ='1' and GEO_TYPE='4'  order by GEO_NAME asc");
$data=$query->rows; 
foreach ($data as $value){
    $insert="insert into ".DB_PREFIX . "all_district (SID,GEO_NAME,GEO_TYPE,Nation_ID,STATE_ID,ACT,statusl) values('".$mcrt->encrypt($value["SID"])."','".$mcrt->encrypt($value["GEO_NAME"])."','".$mcrt->encrypt($value["GEO_TYPE"])."','".$mcrt->encrypt($value["Nation_ID"])."','".$mcrt->encrypt($value["STATE_ID"])."','".$mcrt->encrypt($value["ACT"])."','0')";
    $stmt = $file_db->prepare($insert);
    $stmt->execute();
}
...
$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "farmer where CR_DEALER = '".$id."' ");
$data=$query->rows;
}
