...
class Modelfarmerregister extends Model {
    ...
    public function CallUsr(){
        $sql="select SID,`NAME` from voc_user where ACT=1 limit 2";
        $query = $this->db->query($sql);
        return $query->rows;   
    }
    ...
    public function SaveFormData($data){
        ...
        $log=new Log("SaveFormData".date('d_m_Y').".log");
        $log->write($data);
    //***************** TOP SELECT BAR DATA ****************// 
    ...
    //****************** DEALER TAB DATA *******************//
        ...
        $deladd   = $data['del-add'];
        $deladd=str_replace("'", "", $deladd);
        $delpin   = $data['del-pin'];
        $deladvcode=$data['del-adv-code'];
        $delmfmcode=$data['del-mfm-code'];
        $delremarks=$data['del_remarks']; 
        $delremarks=str_replace("'", "", $delremarks);
...
//************************************** CHECK DATA AND INSERT UPDATE STARTED***********************// 
	...
	$cr_datetime=date("Y-m-d H:i:s");
        ...
        //******************************STATUS UPDATE WITHOUT FARMER/DEALER DATA*******************//
        if($calsts==4 ||
           $calsts==5 || 
           $calsts==6 || 
           $calsts==11 || 
           ...
           $calsts==23 && $empsts=="0"){
            ...
            $log=new Log("SingleStatus".date('d_m_Y').".log");
        ...
        //*********************STATUS UPDATE WITHOUT FARMER/DEALER DATA END*******************//
        
        //************************COMPLAINTS WITH FARMER DATA STARTED ************************//
        
        if($calsts==2 && $empsts==1)//COMPLAINT + FRAMER
        {
            $log->write("In Complaint + Farmer Section");
            $log=new Log("FarmerComplaint".date('d_m_Y').".log");
            ...
            $sql="insert into ak_farmer set 
                ...
                if($ret_id==1)
                {
                    ...
                    if($query->row['MAP_TYPE']==1){//if map type=1
                        ...
                        $ra_days=$queryraaa->row['RA_DAYS'];
                        $ra_id=$queryraaa->row['RA_ID'];
                        if($ra_id==0){//NO Resolution Authority
                            $akra="select customer_id from ak_customer where region_id='".$region."' and customer_group_id=4";
                            $log->write("Get_RA_FROM_AK_CUSTOMER : ".$akra);
                            $queryra=$this->db->query($akra);
                            $ra_id=$queryra->row['customer_id'];
                        ...
                        $aa_days=$queryraaa->row['AA_DAYS'];
                        $aa_id=$queryraaa->row['AA_ID'];
                        if($aa_id==0){//NO Approval Authority
                            $akaa="select customer_id from ak_customer where zone_id='".$zone."' and customer_group_id=3";
                            $log->write("Get_AA_FROM_AK_CUSTOMER : ".$akaa);
                            $queryra=$this->db->query($akaa);
                            $aa_id=$queryra->row['customer_id'];
                    ...
                    if($query->row['MAP_TYPE']==5){//if map type=5 Mapped On August 11
                        ...
                        if((empty($query->row['PRODUCT_CATG']))||($query->row['PRODUCT_CATG']==$procat)){
                            ...
                            if($querygt->row['geo_type']==10){// RA Mapped By Zone
                    ...
                    $aa_due_date=$due_date;
                        ...
                        if(empty($ra_id) || $ra_id=='')//No Complaint Registered As Ra Not Assigned
                        {
                            return 2;
                        }
                        else{//Complaint Registered Start
        ...
        //
        //
        //*********************** COMPLAINTS WITH DEALER DATA START**************************//
        if($calsts==2 && $empsts==2){
            
            $log->write("In Complaint + Dealer Section");
            $log=new Log("DealerComplaint".date('d_m_Y').".log");
            // find zone & region from state id 
            $zrsql="SELECT ZO_ID, RO_ID FROM mas_pol_geo WHERE GEO_ID='".$delst."'";
            $log->write("Find Zone & Region: ".$zrsql);
            $query = $this->db->query($zrsql);
            $delzo=$query->row['ZO_ID'];
            $delro=$query->row['RO_ID'];
            //Check Whether Dealer record exist or not
            $chksql="select `DEL_MOB` from `ak_dealer` where `DEL_MOB`='".$delmob."'";
            $log->write("Check Dealer Data : ".$chksql);
            $query = $this->db->query($chksql);
            if($query->row['DEL_MOB']){
                //Update ak_dealer start
                $insql="UPDATE `ak_dealer` SET
                    `DEL_ID` ='".$far_id."',
                    `DEL_FIRM_NAME`='".$delfirm."',
                    `DEL_NAME` ='".$delname."',
                    `STATE_ID` ='".$delst."',
                    `DISTRICT_ID`='".$deldt."',
                    `ZO_ID` ='".$delzo."',
                    `RO_ID` ='".$delro."',
                    `DEL_LOC`='".$deladd."',
                    `DEL_ADV_CODE` ='".$deladvcode."',
                    `DEL_MFMS_CODE` ='".$delmfmcode."',
                    `CALL_STATUS` ='".$calsts."',
                    `EMP_STATUS`='".$empsts."',
                    `CR_DATE` ='".$cr_datetime."',
                    `CR_BY` ='".$cr_by."',`DEL_CODE`='".$ApiFarmerCode."', `DEL_LIVE`= '".$ApiFarmerLive."',`DEL_TYPE`='".$ApiFarmerType."',`DEL_CAT`='".$delcat."' WHERE `DEL_MOB` ='".$delmob."'";
            }
            else{
                //Insert Into ak_dealer start
                $insql="INSERT INTO `ak_dealer` SET
                    `DEL_ID` ='".$far_id."',
                    `DEL_FIRM_NAME`='".$delfirm."',
                    `DEL_NAME` ='".$delname."',
                    `DEL_MOB` ='".$delmob."',
                    ...
                    `EMP_STATUS`='".$empsts."',
                    `CR_DATE` ='".$cr_datetime."',
                    `CR_BY` ='".$cr_by."',`DEL_CODE`='".$ApiFarmerCode."', `DEL_LIVE`= '".$ApiFarmerLive."',`DEL_TYPE`='".$ApiFarmerType."',`DEL_CAT`='".$delcat."'";
            }
        ...
        //*********************** COMPLAINTS WITH DEALER DATA END **************************//
        
        
        //******************** AGRO ADVISORY WITH DATA FARMER START ************************//
        if($calsts==32 && $empsts==1)//AGRO ADVISORY + FARMER
        {
            $log->write("In Agro + Farmer Section");
            $log=new Log("FarmerAgro".date('d_m_Y').".log");
             //Check Farmer Data exist Or Not
            $chkfrm="select `FAR_MOBILE` from ak_farmer where `FAR_MOBILE` ='".$farmermobile."'";
            $log->write("Check far Exist : ".$chkfrm);
            $chkrow=$this->db->query($chkfrm);
            ...
            $sql="insert into ak_farmer set 
                ...
                if($ret_id==1)
                {
                    ...
                    //CHECK IF THERE IS AN OPEN CASE FOR THIS FARMER
                    $chkpin="SELECT CASE_PIN FROM crm_adv WHERE FAR_MOB='".$farmob."' AND CASE_STATUS=7";
                    $query = $this->db->query($chkpin);
                    $open_case = $query->row['CASE_PIN'];
                    if(empty($open_case)){
        ...
        //******************** AGRO ADVISORY WITH DATA FARMER END ************************//
        //******************** AGRO ADVISORY WITH DEALER DATA START ************************//
        if($calsts==32 && $empsts==2)//AGRO ADVISORY + DEALER
        {
            $log->write("In Agro + Dealer Section");
            $log=new Log("DealerAgro".date('d_m_Y').".log");
            // find zone & region from state id 
            $zrsql="SELECT ZO_ID, RO_ID FROM mas_pol_geo WHERE GEO_ID='".$delst."'";
            $log->write("Find Zone & Region: ".$zrsql);
            $query = $this->db->query($zrsql);
        ...
        //******************** AGRO ADVISORY WITH DEALER DATA END ************************//
        
        //******************** PRODUCT SERVICES WITH  FARMER DATA START ************************//
        ...
        {
            $log->write("Services + Farmer Section");
            $log=new Log("FarmerServices".date('d_m_Y').".log");
             //Check Farmer Data exist Or Not
            $chkfrm="select `FAR_MOBILE` from ak_farmer where `FAR_MOBILE` ='".$farmermobile."'";
            $log->write("Check far Exist : ".$chkfrm);
            $chkrow=$this->db->query($chkfrm);
          ...
          //******************** PRODUCT SERVICES WITH  FARMER DATA END ************************//
           //******************** PRODUCT SERVICES WITH  DEALER DATA START ************************//
        
        if(($calsts==29 ||$calsts==30 ||$calsts==31 ||$calsts==33 ||$calsts==34  ||$calsts==36) && $empsts==2)//AGRO PRODUCT, NETWORK, SERVICE, SUGGESTION, FEEDBACK + FARMER
        {
            $log->write("Services + Dealer Section");
            $log=new Log("DealerServices".date('d_m_Y').".log");
            $zrsql="SELECT ZO_ID, RO_ID FROM mas_pol_geo WHERE GEO_ID='".$delst."'";
            $log->write("Find Zone & Region: ".$zrsql);
            $query = $this->db->query($zrsql);
            $delzo=$query->row['ZO_ID'];
