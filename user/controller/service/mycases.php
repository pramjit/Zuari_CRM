<?php
class Controllerapprovalmycases extends Controller {
public function  index(){
//$this->load->language('approval/mycases');
...
$data['lnk'] = $this->url->link('approval/mycases','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['MyCaseData']= $this->model_approval_mycases->getmissedcallData($this->request->get);
$order_total_count = $this->model_approval_mycases->getmissedcallDatacount($this->request->get);
$order_total = count($order_total_count);

$data['StateData']= $this->model_approval_mycases->StateData();
$data['CropData']= $this->model_approval_mycases->CropData();
$data['CallSts']= $this->model_approval_mycases->CallSts();
$data['CallUsr']= $this->model_approval_mycases->CallUsr();
$data['ProdCatData']= $this->model_approval_mycases->ProdCatData();
$data['CompCatData']= $this->model_approval_mycases->CompCatData();
$data['EnqCatData']= $this->model_approval_mycases->EnqCatData();
$data['EnqTypData']= $this->model_approval_mycases->EnqTypData();
...
$pagination->url = $this->url->link('approval/mycases', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['order_statuses'] = "";
$this->session->data["title"]='Approval Authority - My Cases';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'My Cases',
'href' => $this->url->link('approval/mycases')
);
...
$this->response->setOutput($this->load->view('default/template/approval/mycases.tpl', $data));
...
public function distlist(){
   ...
   // $a = $this->request->post['stsid'];
    $DistData= $this->model_approval_mycases->DistData($this->request->post['stsid']);
    //print_r( $DistData);
...
public function prodlist(){
    $this->load->model('approval/mycases');
   // $a = $this->request->post['stsid'];
    $AaStatus= $this->model_approval_mycases->AaStatus();
    $ProdData= $this->model_approval_mycases->ProdData($this->request->post['caseid']);
    //print_r( $DistData); 
        ...
        echo '<th class="text-center">COMPLAIN CATEGORY</th>';
        echo '<th class="text-center">COMPLAIN SUB-CAT</th>';
        echo '<th class="text-center">COMPLAIN DETAILS</th>';
        ...
        echo '<button type="button" class="btn btn-default" onclick="subaadata();">Submit</button>';
   ...
   // $a = $this->request->post['stsid'];
    ...
    $HisData= $this->model_approval_mycases->History($this->request->post['caseid']);
    ...
    public function subaadata(){
        ...
        {   
                $file = $caseid.'_AA_'.rand(1000, 9999).'_'.$_FILES['ra_file']['name']; 
                move_uploaded_file($_FILES['ra_file']['tmp_name'], DIR_UPLOAD . $file);
                $filepath=$file;
        } 
        
        $AaSubData= $this->model_approval_mycases->subaadata($caseid,$commnts,$status,$filepath);
        if($AaSubData==1)
        {
