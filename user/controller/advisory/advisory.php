...
$this->request->get['limit' ]=$this->config->get('config_limit_admin');
 ...
 $data['statuslist']= $this->model_advisory_advisory->getStatus($this->request->get);         
...
{
        $this->load->model('advisory/advisory');
        $log=new Log("updatestaus.log");
        $log->write($this->request->post);
        $Data= $this->model_advisory_advisory->updateStatus($this->request->post);
