<?php
class Controllercalladvisory extends Controller {
public function index(){
//$this->load->language('call/advisory');
$this->document->setTitle($this->language->get('heading_title'));      
$this->load->model('call/advisory');
...
$data['lnk'] = $this->url->link('call/advisory','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['AdvData']= $this->model_call_advisory->AdvData($this->request->get);
$order_total_count = $this->model_call_advisory->getmissedcallDatacount($this->request->get);
$order_total = count($order_total_count);

$data['StateData']= $this->model_call_advisory->StateData();

$data['CropData']= $this->model_call_advisory->CropData();
$data['CallSts']= $this->model_call_advisory->CallSts();
$data['CallUsr']= $this->model_call_advisory->CallUsr();
$data['ProdCatData']= $this->model_call_advisory->ProdCatData();
$data['CompCatData']= $this->model_call_advisory->CompCatData();
$data['EnqCatData']= $this->model_call_advisory->EnqCatData();
$data['EnqTypData']= $this->model_call_advisory->EnqTypData();
...
$pagination->url = $this->url->link('call/advisory', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['order_statuses'] = "";
$this->session->data["title"]='Advisory Data';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'Advisory Call',
'href' => $this->url->link('call/advisory')
);
...
$this->response->setOutput($this->load->view('default/template/call/advisory.tpl', $data));
...
public function distlist(){
   ...
   // $a = $this->request->post['stsid'];
    $DistData= $this->model_call_advisory->DistData($this->request->post['stsid']);
    //print_r( $DistData);
...
public function prodlist(){
   ...
   // $a = $this->request->post['stsid'];
    ...
    $ProdData= $this->model_call_advisory->ProdData($this->request->post['catid']);
    //print_r( $DistData);
...
public function complist(){
    $this->load->model('call/advisory');
   // $a = $this->request->post['stsid'];
    $CompData= $this->model_call_advisory->CompData($this->request->post['comid']);
    //print_r( $DistData);
	...
	//*****************************MAIL FUNCTION END***********************************************//
        $SaveData= $this->model_call_advisory->SaveFeedBack($this->request->post);
        ...
        if($SaveData==0){
            ...
            $CaseMail= $this->model_call_advisory->CaseMailDtls($SaveData);
           ...
           // $raid   =   $CaseMail['RAID'];  //Ra Id
            ...
            $sub='Advisory : '.$days.' awaiting closure';
            $msg='Dear Sir,<br><br><br>
			Resolution for Advisory '.$days.' has been transcribed by the call centre resource based on the recording of the conversation between you and the caller.<br><br>
                        Kindly review through app or web and provide closure and if required please do the necessary changes.<br><br><br>
    ...
    public function advrecord(){
        ...
        $RecData= $this->model_call_advisory->RecData($this->request->post);
        if($RecData){
    ...
    public function reassign(){
        $this->load->model('call/advisory');
        $log=new Log("ReAssign".date('d_m_Y').".log");
        $log->write($this->request->post);
        $ReAdvisory= $this->model_call_advisory->ReAdvisory($this->request->post['caseid']);
        if($ReAdvisory==1){
            echo 'Case Re - Assigned Successfully';
        }
        else{
            echo 'Kindly Report to administrator';
        }
    }
