 ...
 * @author agent
 */
class Controllerdfcsurveyreport extends Controller {
    public function  index(){
        
        //$this->load->language('report/attendancereport');
    ...
    protected function getList() {
        ...
        if (isset($this->request->get['farmerid'])) {
            $farmerid = $this->request->get['farmerid'];
        }

        if (isset($this->request->get['lastso'])) {
            $lastso = $this->request->get['lastso'];
        } 
         if (isset($this->request->get['lastfarmerid'])) {
            $lastfarmerid = $this->request->get['lastfarmerid'];
        } 
         if (isset($this->request->get['surveydate'])) {
            $surveydate = $this->request->get['surveydate'];
        } 
        if (isset($this->request->get['lastfromdate'])) {
            $lastfromdate = $this->request->get['lastfromdate'];
        } 
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['lnk'] = $this->url->link('techfarmer/customerfarmer', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $data['lastfarmerid']=$lastfarmerid;
      
       
         $data['lastfromdate']=$lastfromdate;
       
     
          $qid = $this->model_techfarmer_customerfarmer->getlastqid($filter_data);
        $qid1 = $this->model_techfarmer_customerfarmer->getlastqid1($filter_data);
        $data = $this->model_techfarmer_customerfarmer->getsurveydetails($filter_data);
        $data1 = $this->model_techfarmer_customerfarmer->getsurveydetails2($filter_data);
        $data2 = $this->model_techfarmer_customerfarmer->getsurveydetails3($filter_data);
        $order_total_new = $this->model_techfarmer_customerfarmer->getsurveydetailscount($filter_data);
       /* foreach($data["surveydata"]  as $survey){
           // array_push($data["surveyfinal"][$survey['SBL_QUEST_ID']], $survey);
           if(key_exists($survey['SBL_QUEST_ID'], $data["surveyfinal"]))
           {
                 array_push($data["surveyfinal"][$survey['SBL_QUEST_ID']], $survey);
             
           }  else {
             $data["surveyfinal"][$survey['SBL_QUEST_ID']]=array($survey);
           }
       ...
       //print_r($data1);
        $i=1;
        $survey['SBL_QUEST_ID']=array();
        foreach($data  as $survey){
             array_push($data3, $survey);
          if($survey["SBL_QUEST_ID"]==$qid) {
              foreach($data1  as $survey1){
              array_push($data3, $survey1);
              }
          }
          if($survey["SBL_QUEST_ID"]==$qid1) {
              foreach($data2  as $survey2){
              array_push($data3, $survey2);
              }
          }
        $i++; }
       // print_r($data3);
         $data["surveydata"]=$data3;
 //print_r($data["surveydata"]);die;
        $order_total=count($order_total_new);
	...
	$this->load->model('report/searchattendance');
          ...
          $data['lastso']=$this->request->get['lastso'];
        ...
        $pagination->url = $this->url->link('techreport/customerfarmer', 'token=' . $this->session->data['token'] . $url . '&page={page}&filter_state_id='.$filter_state_id.'&filter_mdo_id='.$filter_mdo_id.'&filter_act_id='.$filter_act_id, 'SSL');
      ...
      //  $this->load->model('localisation/order_status');
        ...
        $this->session->data["title"]='Customer';
        $data['sort'] = $sort;
        $data['order'] = $order;
		...
		$data['breadcrumbs'][] = array(
			'text' => 'customer',
			'href' => $this->url->link('techreport/customerfarmer', 'token=' . $this->session->data['token'], 'SSL')
		);
      ...
      $this->response->setOutput($this->load->view('default/template/dfc/surveyreport.tpl', $data));
    ...
    echo '<option value="">All MDO</option>';
