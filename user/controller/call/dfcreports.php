<?php
class Controllerdfcdfcreports extends Controller {
public function  index(){
$this->load->language('dfc/dfcreports');
...
$data['lnk'] = $this->url->link('dfc/dfcreports','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['dfcdata']= $this->model_dfc_dfcreports->getdfcdata($this->request->get);
$order_total_count = $this->model_dfc_dfcreports->getdfcdatacount($this->request->get);
...
$pagination->url = $this->url->link('dfc/dfcreports', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['breadcrumbs'][] = array(
'text' => 'DFC Reports',
'href' => $this->url->link('dfc/dfcreports')
);
...
$this->response->setOutput($this->load->view('default/template/dfc/dfcreports.tpl', $data));
...
$this->load->model('dfc/dfcreports');
$results = $this->model_dfc_dfcreports->getdfcdata($this->request->get);
$row = 2;
foreach($results as $data)
{
$col = 0;
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $data['soname']);
...
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['REMARKS']);
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['CUSTOMER_POSSITION']);
$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['FAR_SEGMENT']);
...
header('Content-Disposition: attachment;filename="DFC_REPORT_'.date('dMy').'.xls"');
