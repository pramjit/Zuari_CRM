<?php
class ControllerApiMailAaPre extends Controller {
        public function index(){
        $log=new Log("ManualMailAaPre".date('dmy').".log");
...
$this->load->model('api/MailAaPre');
$CompData= $this->model_api_MailAaPre->checkComplain();
$log->write($CompData);
if($CompData){
    ...
    foreach($CompData as $CD){
        ...
        if($raday==1)// One Day Befor AA Date
        {
            ...
            $sub='Reminder for complaint  '.$caseid.' : 1 day left';
            $msg='Dear '.$aaname.', <br><br><br>
            Complaint '.$caseid.' is pending for approval. <br><br>
            One day is left to approve the resolution within cycle time.
            The resolution can be accessed at '.$url.'<br><br><br>
            ...
            $aamsg="Resolution ".$caseid." approval deadline is tomorrow Kindly review through app or web and provide closure. -Customer complaint team";
