...
class ControllerApiLogin extends Controller {
        public  function gcm(){
            ...
            $api_info = $this->customer->sentgcmdata($emp_id,"del",true);
            if($api_info){
                echo '1';
            } else {
                echo '0';
	...
	public function index() {
            ...
            $keys = array('username','password','appid','token');
            /*foreach ($keys as $key) {
		...
		$this->model_account_activity->addActivity('login', $activity_data);
                ...
                if ($api_info) {     
                    if(isset($this->request->post['appid']) && isset($this->request->post['token'])){            						                   
                        $this->customer->updategsmid($this->request->post['appid'],$this->request->post['token'],$this->customer->getId());
                    }
                        ...
                        $cust->email=$mcrypt->encrypt($this->customer->getUserId());
        ...
        public function reset(){ 
            ...
            if($customer_info) {
                ...
                if($checkotp) {
                    ...
                    $ff=$customer_info['customer_group_id'];
                    $customer_info["otp"] = $this->request->post['otp'];
                    $updateotpstatus = $this->model_account_custom_field->updateotpstatus($customer_info);
                    if(($customer_info['customer_group_id']=="64") || ($customer_info['customer_group_id']=="65") || ($customer_info['customer_group_id']=="67") || ($customer_info['customer_group_id']=="68")) {
