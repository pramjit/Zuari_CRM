<?php
class Controllerresolutionmycases extends Controller {
public function  index(){
//$this->load->language('resolution/mycases');
$this->document->setTitle($this->language->get('heading_title'));      
$this->load->model('resolution/mycases');
...
$data['lnk'] = $this->url->link('resolution/mycases','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['MyCaseData']= $this->model_resolution_mycases->getmissedcallData($this->request->get);
$order_total_count = $this->model_resolution_mycases->getmissedcallDatacount($this->request->get);
$order_total = count($order_total_count);

$data['StateData']= $this->model_resolution_mycases->StateData();
$data['CropData']= $this->model_resolution_mycases->CropData();
$data['CallSts']= $this->model_resolution_mycases->CallSts();
$data['CallUsr']= $this->model_resolution_mycases->CallUsr();
$data['ProdCatData']= $this->model_resolution_mycases->ProdCatData();
$data['CompCatData']= $this->model_resolution_mycases->CompCatData();
$data['EnqCatData']= $this->model_resolution_mycases->EnqCatData();
$data['EnqTypData']= $this->model_resolution_mycases->EnqTypData();
...
$pagination->url = $this->url->link('resolution/mycases', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['order_statuses'] = "";
$this->session->data["title"]='Resolution Authority - My Cases';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'My Cases',
'href' => $this->url->link('resolution/mycases')
);
...
$this->response->setOutput($this->load->view('default/template/resolution/mycases.tpl', $data));
...
public function distlist(){
   ...
   // $a = $this->request->post['stsid'];
    $DistData= $this->model_resolution_mycases->DistData($this->request->post['stsid']);
    //print_r( $DistData);
...
public function prodlist(){
    $this->load->model('resolution/mycases');
   // $a = $this->request->post['stsid'];
    $RaStatus= $this->model_resolution_mycases->RaStatus();
    $ProdData= $this->model_resolution_mycases->ProdData($this->request->post['caseid']);
    ...
    //print_r( $DistData); 
        ...
        echo '<thead>';
        echo '<tr role="row" style="background: #12a4f4; color: #ffffff; font-size:12px; font-weight:bold;">';
        echo '<th class="text-center">COMPLAINT STATE</th>';
        echo '<th class="text-center">COMPLAINT DISTRICT</th>';
        echo '<th class="text-center">COMPLAINT CATEGORY</th>';
        echo '<th class="text-center">COMPLAINT SUB-CAT</th>';
        echo '<th class="text-center">COMPLAINT DETAILS</th>';
        echo '<th class="text-center">CASE CREATED BY</th>';
        echo '<th class="text-center">CURRENT OWNER</th>';
        echo '<th class="text-center">HISTORY</th>';
        echo '<th class="text-center">DOWNLOAD</th>';
        echo '<th class="text-center">COMMENTS</th>';
        echo '<th class="text-center">UPLOAD</th>';
        ...
        foreach($ProdData as $prod){
            ...
            if(empty($cur_usr))
            {
                ...
                $cur_usr= $this->model_resolution_mycases->CaseCurUser($this->customer->getId());
            }
            echo '<tr style="font-size:11px;">';
                echo "<td>".$prod['COM_ST']."</td>";
                echo "<td>".$prod['COM_DT']."</td>";
                ...
                echo "<td><select class='form-control' name='ra_sts' id='ra_sts'>";
   ...
   // $a = $this->request->post['stsid'];
    ...
    $HisData= $this->model_resolution_mycases->History($this->request->post['caseid']);
	...
	//*****************************MAIL FUNCTION END***********************************************//
        ...
        $RaSubData= $this->model_resolution_mycases->subradata($caseid,$commnts,$status,$filepath);
        if($RaSubData==1)
        {
            //On successful submit data sent mail to Approval Authority
             $CaseMail= $this->model_resolution_mycases->CaseMailDtls($caseid);
             $m_caseid=$CaseMail['CASE_ID'];
             $m_name=$CaseMail['AA_NAME'];
             $m_mail=$CaseMail['AA_MAIL'];
             $m_aa_days=$CaseMail['NO_DAY'];
             $url= 'zuari.akshapp.com';
                $cc='asad.ahmed@adventz.com';
                $sub="Complaint ".$m_caseid." Approval Required";
                $msg="Dear ".$m_name.",<br><br><br>
                    Complaint ".$m_caseid." resolution comments have been posted by the resolving authority for your approval. <br><br>
                    The complaint with resolution comments can be accessed through ".$url."<br><br>
                    The expected Resolution date for this complaint is ".$m_aa_days." working  days post complaint resolution posting date.<br><br><br><br>
                    
                    Best Regards,<br>
                    Support- Adventz Agri CRM";
                    MsgToMail($m_mail,$cc,$sub,$msg);
         }
        else{
