...
class ControllerAccountForgotten extends Controller {
	...
	protected function validate() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {
			$password = substr(sha1(uniqid(mt_rand(), true)), 0, 10);
			$this->model_account_customer->editPassword($this->request->post['id'], $password);
			$message  = sprintf($this->language->get('text_greeting'), $this->config->get('config_name')) . "\n\n";
			$message .= $this->language->get('text_password') . "\n\n";
			$message .= $password;	
			// Add to activity log
			$customer_info = $this->model_account_customer->getCustomerByEmail($this->request->post['id']);
			if ($customer_info) {
				$this->load->model('account/activity');
             ...
             protected function sentotp(){                               
                 ...
                 $otp = $this->model_account_custom_field->generateotp($this->request->post);
                 if($otp){
