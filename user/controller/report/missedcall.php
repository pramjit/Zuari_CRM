<?php
class Controllerreportmissedcall extends Controller {
public function  index(){
//$this->load->language('report/missedcall');
$this->document->setTitle($this->language->get('heading_title'));      
$this->load->model('report/missedcall');
...
$data['lnk'] = $this->url->link('report/missedcall','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['misscallData']= $this->model_report_missedcall->getmissedcallData($this->request->get);
$order_total_count = $this->model_report_missedcall->getmissedcallData($this->request->get);
...
$pagination->url = $this->url->link('report/missedcall', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['order_statuses'] = "";
$this->session->data["title"]='Miscall Call Report';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'miscall Details',
'href' => $this->url->link('report/missedcall')
);
...
$this->response->setOutput($this->load->view('default/template/report/missedcall.tpl', $data));
...
$this->load->model('report/missedcall');
$results =$this->model_report_missedcall->getmissedcallData($this->request->get);
...
foreach($results as $data)
{
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['STATE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['MOBILE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['DATE_RECEIVED']);
        if($data['KEY_PRESS']==1)
        {
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row,"ADVISORY");
        }else if($data['KEY_PRESS']==2)
        {
           $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row,"FARMER");  
        }
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:D1')->getFont()->getColor()->setRGB('000DEF');
$col++;
$row++;
}
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Missed_Call_Report'.date('dMy').'.xls"');
