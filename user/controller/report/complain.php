<?php
class Controllerreportcomplain extends Controller {
public function  index(){
//$this->load->language('report/complain');
$this->document->setTitle($this->language->get('heading_title'));      
$this->load->model('report/complain');
...
$data['lnk'] = $this->url->link('report/complain','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['misscallData']= $this->model_report_complain->getmissedcallData($this->request->get);
$order_total_count = $this->model_report_complain->getmissedcallDatacount($this->request->get);
...
$pagination->url = $this->url->link('report/complain', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['order_statuses'] = "";
$this->session->data["title"]='Complaints Data';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'Complain Call',
'href' => $this->url->link('report/complain')
);
...
$this->response->setOutput($this->load->view('default/template/report/complain.tpl', $data));
//print_r( $data['mob3']); die;
}

public function misscall_download(){
...
$this->load->model('report/complain');
$results =$this->model_report_complain->getmissedcallData($this->request->get);
$row = 2;
$col = 1;
foreach($results as $data)
{
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['FAR_MOB']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['STATE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['CALL_TYPE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['COM_CASE_STATUS']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['CASE_CR_DATE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['CYCLE_DAYS']);
$col++;
$row++;
}
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Complaint_Call_Report'.date('dMy').'.xls"');
header('Cache-Control: max-age=0');
$objWriter->save('php://output');
}
public function distlist(){
    
   // $a = $this->request->get['stsid'];
    //echo $a;
    echo 'a';
