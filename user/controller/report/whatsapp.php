<?php
class Controllerreportwhatsapp extends Controller {
...
$data['lnk'] = $this->url->link('report/whatsapp','token=' . $this->session->data['token'], 'SSL');
$data['orders'] = array();
...
$data['misscallData']= $this->model_report_missedcall->getwhatsappData($this->request->get);
$order_total_count = $this->model_report_missedcall->getwhatsappData($this->request->get);
...
$pagination->url = $this->url->link('report/missedcall/getwhatsappData', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
$data['order_statuses'] = "";
$this->session->data["title"]='WhatsApp Call Report';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'WhatsApp Call Details',
'href' => $this->url->link('report/whatsapp')
);
...
$this->response->setOutput($this->load->view('default/template/report/whatsapp.tpl', $data));
//print_r( $data['mob3']); die;
}

public function whatsapp_download(){
...
$this->load->model('report/missedcall');
$results =$this->model_report_missedcall->getwhatsappData($this->request->get);
$row = 2;
$col = 1;
foreach($results as $data)
{
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:D1')->getFont()->getColor()->setRGB('0066FF');
$col++;
$row++;
}
