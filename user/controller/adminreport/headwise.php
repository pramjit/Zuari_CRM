<?php
class Controlleradminreportheadwise extends Controller {
...
//start Advisory Head
$this->load->model('adminreport/headwise');
if($this->request->get['from_date'] && $this->request->get['to_date'] ){
  $data['head']= $this->model_adminreport_headwise->getAdvisoryhead($this->request->get);  
}
//$data['head']= $this->model_advisory_headwise->getAdvisoryhead($this->request->post);


$this->response->setOutput($this->load->view('default/template/adminreport/headwise.tpl', $data));
...
$this->load->model('adminreport/headwise');
$results= $this->model_adminreport_headwise->getAdvisoryhead($this->request->get); 
$row = 2;
$col = 1;
foreach($results as $data)
{
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['ADV_NAME']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['STATE_NAME']);
