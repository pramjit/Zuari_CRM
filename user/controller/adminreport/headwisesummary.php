<?php
class Controlleradminreportheadwisesummary extends Controller {
...
//start Advisory Head
$this->load->model('adminreport/headwisesummary');
if($this->request->get['from_date'] && $this->request->get['to_date'] ){
  $data['head']= $this->model_adminreport_headwisesummary->getAdvisoryhead($this->request->get);  
}
//$data['head']= $this->model_advisory_headwise->getAdvisoryhead($this->request->post);


$this->response->setOutput($this->load->view('default/template/adminreport/headwisesummary.tpl', $data));
...
$this->load->model('adminreport/headwisesummary');
$results= $this->model_adminreport_headwisesummary->getAdvisoryhead($this->request->get); 
$row = 2;
$col = 1;
foreach($results as $data)
{
  $totalGrand=$data['Crop Nutrition']+$data['Crop Protection']+$data['Seed']+$data['Soil']+$data['Irrigation']+$data['Others'];
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $col);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['STATE_NAME']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['ADV_NAME']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['Crop Nutrition']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['Crop Protection']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['Seed']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['Soil']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $data['Irrigation']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $data['Others']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $totalGrand);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:J1')->getFont()->setItalic(true);
        $objPHPExcel->getActiveSheet()->getStyle('A1:J1')->getFont()->setBold(true);
        $objPHPExcel->getActiveSheet()->getStyle('A1:J1')->getFont()->setSize(12);
        $objPHPExcel->getActiveSheet()->getDefaultRowDimension('A1:J1')->setRowHeight(20);
        $objPHPExcel->getActiveSheet()->getStyle('A1:J1')->getFont()->getColor()->setRGB('000DEF');
$col++;
$row++;
}
