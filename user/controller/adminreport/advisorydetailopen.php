<?php
class Controlleradminreportadvisorydetailopen extends Controller 
{
...
$data['order_statuses'] = "";
$this->session->data["title"]='Open Details Report';
$data['sort'] = $sort;
$data['order'] = $order;
...
$data['breadcrumbs'][] = array(
'text' => 'Open Details Report',
'href' => $this->url->link('adminreport/advisorydetailopen')
);
$this->load->model('adminreport/advisorydetailopen');
$data['farmerdata']= $this->model_adminreport_advisorydetailopen->farmerdetail($this->request->get);
...
$this->response->setOutput($this->load->view('default/template/adminreport/advisorydetailopen.tpl', $data));
...
$this->load->model('adminreport/advisorydetailopen');
$farmerdata= $this->model_adminreport_advisorydetailopen->farmerdetail($this->request->get);
$row = 2;
$col = 1;
$tp='';
...
{ 
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['ADVNAME']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['TYPE']);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:F1')->getFont()->getColor()->setRGB('000DEF');
$col++;
$row++;
}
