 ...
 * @author agent
 */
class Controllerdelarviewdealer extends Controller {
    public function  index(){
        
        $this->load->language('delar/viewdealer');
    ...
    protected function getList() {
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['retailerdata']= $this->model_delar_delarregistration->getdealerdata();
        $order_total_count = $this->model_delar_delarregistration->getdealerdata();
	...
	$this->document->setTitle($this->language->get('Dealer View'));
	...
	//$this->load->model('retailer/createretailer');
        ...
        $pagination->url = $this->url->link('delar/viewdealer', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      //  $this->load->model('localisation/order_status');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'View Dealer',
			'href' => $this->url->link('delar/viewdealer', 'token=' . $this->session->data['token'], 'SSL')
		);
      ...
      $this->response->setOutput($this->load->view('delar/viewdealer.tpl', $data));
    ...
    $this->load->model('delar/delarregistration');
    $results = $this->model_delar_delarregistration->getdealerdata();
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['address']);
         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['sap_id']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $data['district_id']);
