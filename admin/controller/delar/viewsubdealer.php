 ...
 * @author agent
 */
class Controllerdelarviewsubdealer extends Controller {
    public function  index(){
        
        $this->load->language('delar/viewsubdealer');
    ...
    protected function getList() {
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['lnk'] = $this->url->link('delar/delarregistration', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $data['retailerdata']= $this->model_delar_delarregistration->getsubdealerdata();
        $order_total_count = $this->model_delar_delarregistration->getsubdealerdata();
	...
	$this->document->setTitle($this->language->get('Sub Dealer View'));
	...
	//$this->load->model('retailer/createretailer');
        ...
        $pagination->url = $this->url->link('delar/viewsubdealer', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      //  $this->load->model('localisation/order_status');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'View Sub Dealer',
			'href' => $this->url->link('delar/viewsubdealer', 'token=' . $this->session->data['token'], 'SSL')
		);
      ...
      $this->response->setOutput($this->load->view('delar/viewsubdealer.tpl', $data));
    ...
    public function dealar_download(){
    ...
    $this->load->model('delar/delarregistration');
    $results = $this->model_delar_delarregistration->getsubdealerdata();
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['lastname']);
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['address']);
         $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['sap_id']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['district_id']);
