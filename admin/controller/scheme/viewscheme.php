 ...
 * @author agent
 */
class Controllerschemeviewscheme extends Controller {
    public function  index(){
        
        //$this->load->language('customer/viewcustomer');
	$this->document->setTitle($this->language->get('heading_title'));
      
	$this->load->model('scheme/schememodel');
        //search mobile number
  //$data['retailerdata']= $this->model_scheme_schememodel->getschemename($scheme_name);

  //print_r($data['retailerdata']); die;
        //end search
      
        
        //$data['dp_search_village']=$this->model_village_searchvillage->get_Search_Village();
         $data['scheme'] = $this->model_scheme_schememodel->getsearch();
    ...
    protected function getList() {
     $data['schemelist']= $this->model_scheme_schememodel->getschemename();
     $data['scheme'] = $this->model_scheme_schememodel->getsearch();

    //  $data['dpparty']= $this->model_scheme_schememodel->getschenamename();
 //  $data['mob']=$this->request->get['scheme_name'];
 //  $mob=$data['mob'];
   // print_r($mob); die;
...
/*
  ...
  /*$data['lastschemeid']=$this->model_scheme_schememodel->getschemeid($this->request->get);
  $data['lastfromdate']=$this->request->get["fromdate"];
  $data['lasttodate']=$this->request->get["todate"];
  $data['retailerdata']= $this->model_scheme_schememodel->getschemedata($this->request->get);*/
   $data['lastschemeid']=$this->model_scheme_schememodel->getschemenamebyid($this->request->get);
   $data['lastfromdate']=$this->request->get["fromdate"];
   $data['lasttodate']=$this->request->get["todate"];
   $data['retailerdata']= $this->model_scheme_schememodel->getschemedata($this->request->get);
  

$order_total_count = $this->model_scheme_schememodel->getschemedata($this->request->get);
$order_total = count($order_total_count);
	...
	$this->document->setTitle($this->language->get('View Scheme'));
	...
	//$this->load->model('retailer/createretailer');
        ...
        $pagination->url = $this->url->link('scheme/viewscheme', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
   ...
   $data['results'] = sprintf($this->language->get('text_pagination'), ($order_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($order_total - $this->config->get('config_limit_admin'))) ? $order_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $order_total, ceil($order_total / $this->config->get('config_limit_admin')));
      ...
      //  $this->load->model('localisation/order_status');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'View Scheme',
			'href' => $this->url->link('scheme/viewscheme', 'token=' . $this->session->data['token'], 'SSL')
		);
      ...
      $this->response->setOutput($this->load->view('scheme/viewscheme.tpl', $data));
...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
    ...
    {
     $this->load->model('scheme/schememodel');
    $results = $this->model_scheme_schememodel->getsearch($this->request->post);
    print_r($results);die;
    /*
    $a='<ul id="country-list">';
foreach($results as $value) {

$a.='<li onClick=selectscheme('.'"'.$value["sname"].'"'.');>'.$value["sname"].'</li>';
} 
$a.='</ul>'; 
echo $a;*/
    }
