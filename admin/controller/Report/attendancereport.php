...
class ControllerReportattendancereport extends Controller {
    ...
    protected function getList() {
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['lnk'] = $this->url->link('Report/attendancereport', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $data['lastmdodata']=$this->model_report_searchattendance->getmdodata($data['laststate']);
        
        $results = $this->model_report_searchattendance->getmdoattendance($filter_data);
        $order_total = $this->model_report_searchattendance->getTotalattendance($filter_data);
        $data['state'] = $this->model_report_searchattendance->getstate();
        $data['activity'] = $this->model_report_searchattendance->getactivitytype();

        foreach ($results as $result) {
            $data['geo'][] = array(
                'State'      => $result['state_name'],
                'MARKET'      => $result['hq_name'],
                'Mdo_Name'        => $result['user_name'],
                'CR_Date'    => ($result['cr_date']),
                'ACT'    => ($result['user_activity']),
	...
	$this->load->model('report/searchattendance');
        ...
        $pagination->url = $this->url->link('Report/attendancereport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      $this->response->setOutput($this->load->view('attendancereport/searchattendance.tpl', $data));
