 ...
 * @author agent
 */
class ControllerReportfarmerdlrept extends Controller {
    public function  index(){
        
        $this->load->language('report/farmerdlrept');

	$this->document->setTitle($this->language->get('heading_title'));      
       
	$this->load->model('report/farmerdlrept');
    ...
    protected function getList() {
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['lnk'] = $this->url->link('Report/farmerdlrept', 'token=' . $this->session->data['token'], 'SSL');
       ...
       $data['state'] = $this->model_report_farmerdlrept->getstate();
        if($lastfromdate || $lasttodate  || $lastpos) {
           // $data['state'] = $this->model_milkcollcdailyreport->getstate();
        $results = $this->model_report_farmerdlrept->Farmer_Detail_Report($filter_data);
        $order_total_count = $this->model_report_farmerdlrept->Farmer_Detail_Report_count($filter_data);
       ...
       // print_r($results);
        foreach ($results as $result) {
            $data['geo'][] = array(
                ...
                'FAR_MOBILE'    => $result['FAR_MOBILE'],
                'Date_of_creation'        => $result['Date_of_creation'],
                'No_of_Milking_cow'    => $result['No_of_Milking_cow'],
                'Total_no_of_cows'    => $result['Total_no_of_cows'],
                'Current_supplier'    => $result['Current_supplier'],
                'Daily_milk_in_ltr'    => $result['Daily_milk_in_ltr'],
                'Mdo'    => $result['Mdo'],
        ...
        $data['farmer_name'] = $this->language->get('farmer_name');
        $data['farmer_mobile'] = $this->language->get('farmer_mobile');
        $data['dateof_creation'] = $this->language->get('dateof_creation');
        
          $data['nof_milking_cow'] = $this->language->get('nof_milking_cow');
          $data['nof_total_cow'] = $this->language->get('nof_total_cow');        
          $data['current_supplier'] = $this->language->get('current_supplier');
          $data['daily_milkIn_ltr'] = $this->language->get('daily_milkIn_ltr');
          $data['mdo_name'] = $this->language->get('mdo_name');
          
          $data['market'] = $this->language->get('market');
          $data['milk_collection'] = $this->language->get('milk_collection');
	...
	$this->load->model('report/farmerdlrept');
        ...
        $pagination->url = $this->url->link('Report/farmerdlrept', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      $this->response->setOutput($this->load->view('attendancereport/farmerdlrept.tpl', $data));
    ...
    $mdodata= $this->model_report_farmerdlrept->gethq($territory);
    ...
    $this->load->model('report/farmerdlrept');
    $results = $this->model_report_farmerdlrept->Farmer_Detail_Report_count($filter_data);
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['FAR_MOBILE']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['Date_of_creation']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['No_of_Milking_cow']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['Total_no_of_cows']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $data['Current_supplier']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['Daily_milk_in_ltr']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $data['Mdo']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $data['Market_Name']);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $data['Created_From']);
...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
    ...
    header('Content-Disposition: attachment;filename="Farmer_detail_report_'.date('dMy').'.xls"');
