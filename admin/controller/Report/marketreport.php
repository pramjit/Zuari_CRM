 ...
 * @author agent
 */
class ControllerReportmarketreport extends Controller {
    public function  index(){
        
        $this->load->language('report/marketreport');

	$this->document->setTitle($this->language->get('heading_title'));
      
       
	$this->load->model('report/marketreport');
    ...
    protected function getList() {
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['lnk'] = $this->url->link('Report/marketreport', 'token=' . $this->session->data['token'], 'SSL');
        ...
        if($lastfromdate || $lasttodate || $laststate || $lastmdo) {
        $results = $this->model_report_marketreport->Farmer_report_count($filter_data);
        $order_total = $this->model_report_marketreport->Farmer_report($filter_data);
        $order_total = count($order_total);
        }
        $data['state'] = $this->model_report_marketreport->getstate();
        $data['hq'] = $this->model_report_marketreport->gethq($territory);
	...
	$this->document->setTitle($this->language->get('heading_title'));
        ...
        $pagination->url = $this->url->link('Report/marketreport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      $this->response->setOutput($this->load->view('attendancereport/market_report.tpl', $data));
    ...
    $mdodata=$this->model_report_marketreport->gethq($territory);
    ...
    $this->load->model('report/marketreport');
    $results = $this->model_report_marketreport->Farmer_report($filter_data);
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['count']);
        $row++;
    }
...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
    ...
    header('Content-Disposition: attachment;filename="market_report_'.date('dMy').'.xls"');
