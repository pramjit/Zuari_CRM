 ...
 * @author agent
 */
class ControllerReportfgmdlreport extends Controller {
    public function  index(){
        
        $this->load->language('report/fgmdlreport');

	$this->document->setTitle($this->language->get('heading_title'));      
       
	$this->load->model('report/fgmdlreport');
    ...
    protected function getList() {
       ...
       if (isset($this->request->get['filter_state_id'])) {
        ...
        $data['lnk'] = $this->url->link('Report/fgmdlreport', 'token=' . $this->session->data['token'], 'SSL');
       ...
       $data['state'] = $this->model_report_fgmdlreport->getstate();
        if($lastfromdate || $lasttodate  || $lastpos) {
           // $data['state'] = $this->model_milkcollcdailyreport->getstate();
        $results = $this->model_report_fgmdlreport->Fgm_detail_report($filter_data);
        $order_total_count = $this->model_report_fgmdlreport->Fgm_detail_report_count($filter_data);
        ...
        $data['column_date'] = $this->language->get('column_date');
          ...
          $data['column_vill_name'] = $this->language->get('column_vill_name');
          $data['number_of_farmer'] = $this->language->get('number_of_farmer');        
	...
	$this->load->model('report/fgmdlreport');
        ...
        $pagination->url = $this->url->link('Report/fgmdlreport', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
      ...
      $this->response->setOutput($this->load->view('attendancereport/fgmdlreport.tpl', $data));
    ...
    $mdodata= $this->model_report_fgmdlreport->gethq($territory);
    ...
    $this->load->model('report/fgmdlreport');
    $results = $this->model_report_fgmdlreport->Fgm_detail_report_count($filter_data);
    ...
    foreach($results as $data)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['Fgm_date']);
...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
    ...
    header('Content-Disposition: attachment;filename="Fmgdetail_report_'.date('dMy').'.xls"');
